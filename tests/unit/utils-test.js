...
module('utils', function() {
  test('#flattenMessages', function(assert) {
    ...
    assert.deepEqual(flattenMessages(result), [
      'recipe must have a description',
      'description is too short',
    ...
    assert.deepEqual(flattenMessages(result['recipe.ingredients[]']), [
      'ingredient name required',
      'quantity too small',
      'quantity required',
      'invalid unit'
  ...
  test('#collateMessages', function(assert) {
    ...
    assert.deepEqual(collateMessages(result['recipe.ingredients[]']), [

...
import { isObject, isArray, result, isFunction } from './utils.js';

const {
  keys
} = Object;
async function validate(_input, constraints) {
...
async function validateArray(array, constraints) {
  ...
  const errors = await Promise.all(array.map(object => validateObject(object, constraints(object))));
  return result(errors);
}

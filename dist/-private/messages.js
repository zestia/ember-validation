...
const locale = {
  'big-decimal': {
    ...
    'max-decimal-places': 'Value must have a maximum of {_maxDecimalDigits} decimal places'
  },
...
function defaultMessageFn(path, tokens) {
  return processMessage(get(locale, path), tokens);
}
function processMessage(string, tokens) {
  return string.replace(/{(.*?)}/g, (match, token) => tokens[token]);
}
function setMessageFn(fn) {
  t = fn;
}
function objectProperties(object, prefix = '') {
  const props = {};
  for (const prop in object) {
    props[`${prefix}${prop}`] = object[prop];
  }
  return props;
}
function messageFor(key, value, object, options = {}) {
  const tokens = {
    ...objectProperties(options, '_'),
...
export { defaultMessageFn, messageFor, processMessage, setMessageFn };
